...
function Music({ ffmpeg, fetchFile }) {
  ...
  function saveMusic(audioUrl, file, startTime, endTime) {
    ...
    editor.scene.userData.music.fileInfo = file;
    editor.scene.userData.music.startTime = startTime;
    editor.scene.userData.music.endTime = endTime;
  }

  function saveMusicState() {
    if (editor.scene && editor.scene.userData.music) {
      localStorage.setItem(
        "musicState",
        JSON.stringify(editor.scene.userData.music),
      );
    }
  }
  ...
  cutButton.dom.addEventListener("click", async () => {
    ...
    // editor.scene.userData.music 객체에 시작 및 종료 시간 업데이트
    if (!editor.scene.userData.music) {
      editor.scene.userData.music = {};
    }
    // editor.scene.userData.music.fileInfo = audioUrl;
    console.log("fileInput.dom.files[0]");
    console.log(fileInput.dom.files[0]);
    saveMusic(audioUrl, fileInput.dom.files[0], markerStartTime, markerEndTime);
    ...
    return {
      audioState: {
        ...
        currentTime: audioContext
          ...
          : 0,
        audioUrl: editor.scene.userData.music
          ? editor.scene.userData.music.fileInfo
          : "",
        markerStartTime: editor.scene.userData.music
          ? editor.scene.userData.music.startTime
          : 0,
        markerEndTime: editor.scene.userData.music
          ? editor.scene.userData.music.endTime
          : 0,
  ...
  container.fromJSON = function (json) {
    if (json.audioState) {
      ...
      editor.scene.userData.music.fileInfo = audioState.audioUrl;
      editor.scene.userData.music.startTime = audioState.markerStartTime;
      editor.scene.userData.music.endTime = audioState.markerEndTime;
      if (audioState.audioUrl) {
        audioPlayer.src = audioState.audioUrl;
      }
      startTimeUi.dom.value = formatTime(editor.scene.userData.music.startTime);
      endTimeUi.dom.value = formatTime(editor.scene.userData.music.endTime);
  ...
  function initializeProgressBar(startTime, endTime) {
    // const progressBar = document.getElementById("progressBar");
    if (progressBar) {
      progressBar.max = endTime - startTime;
      progressBar.value = 0;
      // 추가적인 초기화 코드
    } else {
  ...
  function initMusic() {
    ...
    if (musicJson) {
      ...
      // musicJson.file이 Blob 타입인지 확인
      if (musicJson.file) {
        loadMusic(musicJson.file);
      } else {
        console.error("musicJson.file is not a Blob");
        // 필요에 따라 Blob으로 변환하거나 오류 처리
      }
