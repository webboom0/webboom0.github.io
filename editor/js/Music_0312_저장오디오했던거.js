...
function Music({ ffmpeg, fetchFile }, _totalSeconds) {
  ...
  // 셀렉트 박스 변경 이벤트
  musicSelect.dom.addEventListener("change", function (event) {
    ...
    console.log(musicSelect.getValue());
    ...
    fetch(absoluteFilePath)
      ...
      .then((arrayBuffer) => {
        ...
        musicFile = new File([blob], selectedOption.text, {
          type: "audio/mpeg",
        });
  ...
  //   const selectedOption = musicSelect.dom.options[selectedIndex];
  //   console.log(selectedOption);
  //   const selectedFilePath = selectedOption.value;
  //   const absoluteFilePath = `${window.location.origin}/${selectedFilePath}`; // 절대 URL 생성
  //   console.log(absoluteFilePath);
  //   fetch(absoluteFilePath)
  //     .then((response) => response.arrayBuffer())
  ...
  function loadMusicFromServer() {
    const filePath = `files/music/music.mp3`;
    const fileName = "music.mp3"; // 파일명을 지정합니다.
    ...
    fetch(filePath)
      ...
      .then((arrayBuffer) => {
        ...
        if (musicSelect) {
          ...
          // musicSelect.setOptions({ [filePath]: fileName });
          // const option = musicSelect.setOptions({ filePath: fileName });
          // option.text = fileName;
          // option.value = filePath; // 파일 경로를 값으로 저장
          ...
          console.error("Select box with id 'musicSelect' not found.");
        }
      })
      .catch((error) => {
...
*/
  ...
  container.toJSON = function () {
    return {
      audioState: {
        audioUrl: audioPlayer.src, // 선택된 오디오 파일의 경로
  ...
  container.fromJSON = function (json) {
    if (json.audioState) {
      // 저장된 오디오 파일 경로로 로드
      if (json.audioState.audioUrl) {
        fetch(json.audioState.audioUrl)
          .then((response) => response.arrayBuffer())
          .then((arrayBuffer) => audioContext.decodeAudioData(arrayBuffer))
          .then((buffer) => {
            audioBuffer = buffer;
            cutAudioBuffer = buffer;
            audioPlayer.src = json.audioState.audioUrl;

            // 저장된 설정 복원
            startTime = json.audioState.markerStartTime;
            endTime = json.audioState.markerEndTime;
            gainNode.gain.value = json.audioState.volume;

            // UI 업데이트
            initializeUI(buffer, json.audioState.audioUrl.split("/").pop());
          });
      }
    }
  };
  ...
  // UI 초기화 함수
  function initializeUI(buffer, fileName) {
    if (!buffer) return;
    ...
    // 총 재생 시간 표시
    durationText.textContent = secondsToTime(buffer.duration);
    ...
    // 현재 시간 텍스트 초기화
    currentTimeText.textContent = secondsToTime(0);
