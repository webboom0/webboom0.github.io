...
class Render {
  constructor(editor, _totalSeconds, _framesPerSecond, _newWindow = true) {
    ...
    this.scene = new THREE.Scene(); // scene 초기화 추가
    // this.scene.copy(this.editorScene); // 에디터 씬 복사
    // 에디터 씬의 객체들을 복사
    ...
    // 나머지 속성들도 복사
    this.scene.background = this.editorScene.background;
    this.scene.environment = this.editorScene.environment;
    this.scene.userData = JSON.parse(JSON.stringify(this.editorScene.userData));
    ...
    // 미리 배경 객체 로드
    this.isBackgroundLoaded = false; // 배경 로딩 상태 체크용
    // animate 바인딩은 여기서 하지 않음
    this.animate = this.animate.bind(this);
    ...
    this.backgroundObject = null; // 클래스 프로퍼티로 추가
    ...
    loader.load("/files/background.obj", (object) => {
      ...
      this.editorScene.add(this.backgroundObject);
  ...
  animate() {
    ...
    if (this.editorScene.userData.keyframes) {
      this.editorScene.children.forEach((character) => {
        // 배경 객체는 건너뛰기
        if (character.name === "Background") return;
  ...
  animate() {
    ...
    if (this.renderer) {
      ...
      if (
        ...
        !this.scene.children.includes(this.backgroundObject)
      ) {
...
*/
  renderPlay() {
    ...
    // 배경이 로드된 경우에만 애니메이션 시작
    if (this.isBackgroundLoaded) {
      this.animate();
    } else {
      console.log(
        "Waiting for background to load before starting animation...",
      );
    }
    // this.animate();
    this.state = "play";
  ...
  renderClose() {
    ...
    if (this.backgroundObject) {
      this.editorScene.remove(this.backgroundObject);
