...
function SidebarProjectVideo(editor) {
  ...
  /*
  // Keyframe Timeline
  const keyframeContainer = new UIPanel();
  keyframeContainer.setId("keyframe-timeline");
  container.add(keyframeContainer);

  const keyframes = []; // 키프레임 저장 배열
  const keyframeRow = new UIRow();
  keyframeContainer.add(keyframeRow);

  // 키프레임 추가 및 클릭 이벤트
  function addKeyframe() {
    const keyframeButton = new UIButton(`Keyframe ${keyframes.length + 1}`);
    keyframeButton.onClick(() => {
      // 현재 화면을 키프레임에 저장하는 로직
      saveCurrentFrame(keyframes.length);
    });
    keyframeRow.add(keyframeButton);
    keyframes.push(keyframeButton);
  }

  // 화면 저장 함수
  function saveCurrentFrame(index) {
    const canvas = document.querySelector("canvas"); // 현재 캔버스 선택
    const dataURL = canvas.toDataURL(); // 현재 캔버스의 이미지를 데이터 URL로 변환
    keyframes[index].setText(`Keyframe ${index + 1} (Saved)`); // 버튼 텍스트 변경
    // 추가적인 저장 로직을 여기에 구현할 수 있습니다.
    console.log(`Keyframe ${index + 1} saved!`);
  }
  ...
  document.addEventListener("contextmenu", (event) => {
    ...
    const keyframeIndex = Math.floor(
      event.clientX / (window.innerWidth / keyframes.length),
    ); // 클릭한 위치에 따라 키프레임 인덱스 계산
    if (keyframeIndex < keyframes.length) {
      saveCurrentFrame(keyframeIndex);
    }
  });
*/
  // Render
  ...
  renderButton.onClick(async () => {
    ...
    await (async function () {
      ...
      videoSizeStatus.textContent = `( ${formatFileSize(
  ...
  // 키프레임 추가 버튼
  //   const addKeyframeButton = new UIButton("Add Keyframe");
  //   addKeyframeButton.onClick(addKeyframe);
  //   container.add(addKeyframeButton);
