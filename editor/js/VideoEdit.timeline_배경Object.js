...
function VideoEditTimeline(
  ...
  const background = {
    create: function () {
      ...
      // OBJ 파일 로드
      loader.load(
        "/files/background.obj", // OBJ 파일 경로
        (object) => {
          // 이미 Background 객체가 존재하는지 확인
          // editor.scene.children이 존재하는지 먼저 확인
          if (!editor.scene || !editor.scene.children) {
            console.log("Scene or children not initialized yet");
            return;
          }

          console.log("Checking for existing background...");
          console.log("Current scene children:", editor.scene.children);

          // 이미 Background 객체가 존재하는지 확인
          const existingBackground = editor.scene.children.find(
            (child) =>
              child.name === "Background" ||
              (child.userData && child.userData.isBackground),
          );
          ...
          // 로드 성공시 실행되는 콜백
          this.plane = object;
          this.planeId = object.id;
          object.name = "Bakcground";
          object.position.x = -97.17106296034069;
          object.position.y = 0;
          object.position.z = 50.12830519275826;
          object.scale.x = 6.18;
          object.scale.y = 7.46;
          object.scale.z = 9.88;
          ...
          // 객체를 키프레임 데이터에서 제외하기 위해 userData에 플래그 추가
  ...
  console.log("hasBackground:", editor.scene.background);

  // 초기화 시점에 userData 초기화
  if (!editor.scene.userData) {
    editor.scene.userData = {};
  }

  // 백그라운드가 아직 생성되지 않은 경우에만 create 호출
  ...
  function onload() {
    ...
    Object.keys(keyframes).forEach((character) => {
      // 배경 객체는 제외
      // const obj = editor.scene.children.find(
      //   (child) => child.uuid === character,
      // );
      // if (obj && obj.userData.excludeFromTimeline) {
      //   return;
      // }
